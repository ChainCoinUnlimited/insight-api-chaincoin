...
BlockController.prototype.block = function(req, res, next) {
  ...
  if (blockCached) {
    ...
    elf.node.services.bitcoind.getBlockHeader(hash, function(err, info) {
      if (err) {
        return self.common.handleErrors(err, res);
      }
...
BlockController.prototype.getBlockReward = function(height, cb) {
  var self = this;
//  var nBudgetPaymentsStartBlock= 4100; //testnet
  var nBudgetPaymentsStartBlock = 1362000; //livenet
  var nSubsidyHalvingInterval = 700800;

  // Force block reward to zero when right shift is undefined.
  var halvings = Math.floor(height / nSubsidyHalvingInterval);
  if (halvings >= 10) {
    return 0.001;
  }

  // Subsidy is cut in half every 700,800 blocks which will occur approximately every 4 years.
  var nSubsidy = 16;
  for (var i = nSubsidyHalvingInterval; i <= height; i += nSubsidyHalvingInterval) {
     nSubsidy -= nSubsidy / 2;
  }

  // Hard fork to reduce the block reward by 10 extra percent (allowing budget/superblocks)
  var nSuperblockPart = (height > nBudgetPaymentsStartBlock) ? nSubsidy/10 : 0;
  var nReward = nSubsidy - nSuperblockPart;

  cb(null, parseFloat(nReward.toString(10)).toFixed(8));
